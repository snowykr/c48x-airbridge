...
const (
	airSaneWorkDir     = "/tmp/c48x-airbridge/airsane"
	airSaneSourceDir   = airSaneWorkDir + "/source"
	airSaneStagePrefix = airSaneWorkDir + "/stage/usr/local"
...
func buildAirSaneSetupPlan(options setupOptions) airSaneSetupPlan {
	...
	return airSaneSetupPlan{
		...
		Steps: []HostStep{
			NewPrivilegedCommandStep("install AirSane build dependencies", "apt-get", "install", "-y", "git", "cmake", "g++", "make", "pkg-config", "libsane-dev", "libjpeg-dev", "libpng-dev", "libavahi-client-dev", "libusb-1.0-0-dev", "curl", "avahi-utils", "rsync"),
			NewCommandStep("reset AirSane build workspace", "rm", "-rf", airSaneWorkDir),
			NewCommandStep("create AirSane build workspace", "mkdir", "-p", airSaneWorkDir),
			NewCommandStep("clone AirSane source", "git", "clone", "--no-checkout", source.Repo, airSaneSourceDir),
			NewCommandStep("fetch pinned AirSane commit", "git", "-C", airSaneSourceDir, "fetch", "--tags", "origin", source.Commit),
			NewCommandStep("checkout pinned AirSane commit", "git", "-C", airSaneSourceDir, "checkout", "--detach", source.Commit),
			NewCommandStep("configure AirSane build", "cmake", "-S", airSaneSourceDir, "-B", airSaneSourceDir+"/build", "-DCMAKE_BUILD_TYPE=Release"),
			NewCommandStep("build AirSane", "cmake", "--build", airSaneSourceDir+"/build", "--parallel"),
			NewCommandStep("stage AirSane install", "cmake", "--install", airSaneSourceDir+"/build", "--prefix", airSaneStagePrefix),
			NewPrivilegedCommandStep("install staged AirSane files", "rsync", "-a", airSaneStagePrefix+"/", "/usr/local/"),
